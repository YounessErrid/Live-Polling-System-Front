...
import { addPoll, updatePoll, resetPollStatus } from "../store/pollsSlice";
import { pollSchema } from "../utils/schemas/pollSchema";

const PollForm = ({ mode = "create", initialData = null, onSuccess }) => {
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      ...
      setChoices(
        Array.isArray(initialData.choices)
          ? initialData.choices.map((c) =>
              typeof c === "string" ? { choice_text: c } : { choice_text: c.choice_text }
            )
          : []
      );
    }
  ...
  const handleAddChoice = (e) => {
    e.preventDefault();
    setChoices([...choices, { choice_text: "" }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === "edit" && initialData?.id) {
      await dispatch(updatePoll({ id: initialData.id, poll: pollData }));
    } else {
      await dispatch(addPoll(pollData));
    }
      ...
      <div className="grid gap-4 mb-4 grid-cols-2">
        <div className="col-span-2">
          ...
          <input
            ...
            required
          />
          {formErrors.question && <p className="text-red-500">{formErrors.question}</p>}
        </div>

        <div className="col-span-2">
          <div className="flex items-center justify-between mb-4">
            <label htmlFor="Choices" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
              Poll Choices
            </label>
            <button type="button" onClick={handleAddChoice} className="text-white bg-blue-700 hover:bg-blue-800 px-5 py-2.5 rounded-lg">
              Add Choice
            </button>
          </div>
          ...
          {formErrors.choices && <p className="text-red-500">{formErrors.choices}</p>}
        </div>
      </div>

      <div className="flex items-center justify-end">
        <button type="submit" disabled={loading} className="text-white bg-green-700 hover:bg-green-800 px-5 py-2.5 rounded-lg">
          {mode === "edit" ? "Update Poll" : "Create Poll"}
        </button>
        <button type="button" onClick={handleResetForm} className="ms-3 text-sm text-gray-700 px-5 py-2.5 border border-gray-300 rounded-lg">
          Cancel
        </button>
      </div>

      {loading && <p className="text-sm text-blue-500">Saving poll...</p>}
      {error && <p className="text-sm text-red-500">Error: {error}</p>}
      {status === "success" && <p className="text-sm text-green-500">Poll saved successfully!</p>}
    </form>
  );
};
